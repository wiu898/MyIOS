...
@interface WSTopDetailController()<UITableViewDataSource,
    UITableViewDelegate>
...
- (void)viewDidLoad{
    ...
    [self.bgImgView sd_setImageWithURL:[NSURL URLWithString:self.topic.picurl] placeholderImage:[UIImage imageNamed:@"comment_profile_default"]];
...
- (void)loadDataWithCache:(BOOL)cache{
    ...
    [WSTopicDetail topDetailWithIndex:self.topicIndex isCache:cache expertID:self.topic.expertId getDataSuccess:^(NSArray *dataArr) {
        ...
        if (weakSelf.topicIndex == 0) {
            [weakSelf.topicDeails removeAllObjects];
        }
