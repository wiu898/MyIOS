...
+ (NSURLSessionDataTask *)GETJSON:(NSString *)urlStr GetDataType:(WSGETDataType)type progress:(progressBlock)progress success:(successBlock)success failure:(failureBlock)failure{
    ...
    return [self GETJSON:urlStr progress:progress
        success:success failure:failure];
}

+ (NSURLSessionDataTask *)GETJSON:(NSString *)urlStr
    progress:(progressBlock)progress
    success:(successBlock)success failure:(failureBlock)failure{
    
    WSGetDataTool *tool = [self shareGetDataTool]; //利用线程初始化
    
    //设置Json解析的,可以接收的,服务器返回类型(Content-Type)
