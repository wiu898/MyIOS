...
@property (strong, nonatomic) NSMutableDictionary *colunMaxYDic;
...
/**
 * 初始化layout后自动调用,可以在该方法中初始化一些自定义的变量参数
 */
- (void)prepareLayout{
...
//当边界发生改变时,是否应该发生新布局。YES则在边界变化时,将重新计算需要布局的信息。
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CGFloat width = (self.collectionView.frame.size.width -
        ...
        (self.colCount - 1) * self.itemSpacing) / self.colCount;
    ...
    CGFloat x = self.sectionInset.left +
        (width +self.itemSpacing) * [minCol intValue];
