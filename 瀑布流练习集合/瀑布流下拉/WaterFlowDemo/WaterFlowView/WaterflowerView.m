...
@implementation WaterflowerView
...
- (void)reloadData{
    ...
    //间距
    CGFloat topM = [self.delegate respondsToSelector:@selector(waterflowerView:marginForType:)] ? [self.delegate waterflowerView:self marginForType:WaterflowerViewMarginTypeTop] : DefaultMargin;
    
    CGFloat bottomM = [self.delegate respondsToSelector:@selector(waterflowerView:marginForType:)] ? [self.delegate waterflowerView:self marginForType:WaterflowerViewMarginTypeBottom] : DefaultMargin;
    
    CGFloat leftM = [self.delegate respondsToSelector:@selector(waterflowerView:marginForType:)] ? [self.delegate waterflowerView:self marginForType:WaterflowerViewMarginTypeLeft] : DefaultMargin;
    
    CGFloat rightM = [self.delegate respondsToSelector:@selector(waterflowerView:marginForType:)] ? [self.delegate waterflowerView:self marginForType:WaterflowerViewMarginTypeRight] : DefaultMargin;
    
    CGFloat columnM = [self.delegate respondsToSelector:@selector(waterflowerView:marginForType:)] ? [self.delegate waterflowerView:self marginForType:WaterflowerViewMarginTypeCulomn] : DefaultMargin;
    
    CGFloat rowM = [self.delegate respondsToSelector:@selector(waterflowerView:marginForType:)] ? [self.delegate waterflowerView:self marginForType:WaterflowerViewMarginTypeRow] : DefaultMargin;
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < numberOfCells; i++) {
        ...
        WaterflowerViewCell *cell = self.showingCells[@(i)];
...
- (id)dequeueReusableCellWithIdentifier:(NSString *)identifier{
    ...
    __block WaterflowerViewCell *reusableCell = nil;
    
    [self.reusableCells enumerateObjectsUsingBlock:^(WaterflowerViewCell *cell, BOOL *stop) {
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    [self.showingCells enumerateKeysAndObjectsUsingBlock:^(id key, WaterflowerViewCell *cell, BOOL *stop) {
