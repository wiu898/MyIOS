...
- (void)setDanmaku:(NSString *)danmaku{
    ...
    if ([NSKeyedUnarchiver unarchiveObjectWithFile:self.path]) {
        ...
        NSDictionary *attr = [manager attributesOfItemAtPath:self.path error:nil];
        
        NSDateFormatter *df = [[NSDateFormatter alloc] init];
        
        [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        
        NSMutableString *timeStr = [NSMutableString stringWithFormat:@"%@",[attr valueForKey:@"NSFileCreationDate"]];
        
        for (NSInteger i = 0; i < timeStr.length; i++) {
            
            if ([timeStr characterAtIndex:i] == '+') {
                
                [timeStr deleteCharactersInRange:NSMakeRange(i, timeStr.length - i)];
                break;
            }
        }
...
- (void)getTheDanModel{
    ...
    NSURLRequest *request = [NSURLRequest requestWithURL:URLD];
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        for (DanMuModel *danmu in obj) {
            ...
            NSMutableAttributedString *contentStr = [[NSMutableAttributedString alloc]
                initWithString:danmu.text.length > 0 ? danmu.text : @"2333"attributes:
                 @{NSFontAttributeName : [UIFont systemFontOfSize:danmu.fontSize ? [danmu.fontSize floatValue] : 14.0],NSForegroundColorAttributeName : UIColorFromRGB(danmu.textColor ? ((long)[danmu.textColor doubleValue]) : 0xffffff)}];
...
- (BOOL)shouldAutorotate{
    ...
    return NO;     //NS_AVAILABLE_IOS(6_0);当前viewcontroller是否支持转屏
}
...
- (BOOL)prefersStatusBarHidden{
    ...
    return NO;   //返回NO表示要显示,返回YES是hidden
}
...
- (void)Stack{
    ...
    if (_playerItem.duration.timescale !=0 ) {
        ...
        //duration 总时长
        NSInteger durMin = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale / 60;   //总秒数
