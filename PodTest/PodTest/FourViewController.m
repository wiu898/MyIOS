...
- (void)viewDidLoad{
    ...
    _loopView.autoScrollTimeInterval = 4;
...
- (void)startDownLoad{
    ...
    __weak FourViewController *weakSelf = self;
...
- (void)dealCoach:(UIButton *)sender{
    QuestionBankViewController *questionBank =
        [[QuestionBankViewController alloc] init];
...
- (void)dealSignUp:(UIButton *)sender{
    QuestionTestViewController *questionTest = [[QuestionTestViewController alloc]
         init];
...
- (void)dealCard:(UIButton *)sender{
    ...
    wrongQuestion.questionerrorurl = self.questionerrorurl;
    if (![AccountManager isLogin]) {
