...
@interface SignUpDrivingViewController()<UITableViewDataSource,
    UITableViewDelegate,BMKLocationServiceDelegate,JENetwokingDelegate>
...
- (UITableView *)tableView{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:
            CGRectMake(0, 64, kSystemWide, kSystemHeight-64) style:UITableViewStylePlain];
...
- (void)clickRight:(UIButton *)sender{
    ...
    if (![AccountManager manager].applyschool.infoId) {
        if (self.detailModel.schoolid || self.detailModel.name) {
            NSDictionary *schoolParam = @{kRealSchoolid:self.detailModel.schoolid,
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:
   (NSIndexPath *)indexPath{
    SignUpDrivingDetailViewController *SelectVC =
       [[SignUpDrivingDetailViewController alloc]init];
    DrivingModel *model = self.dataArray[indexPath.row];
