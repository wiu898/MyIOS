    ...
    [JENetwoking startDownLoadWithUrl:urlString postParam:nil
        WithMethod:JENetworkingRequestMethodGet withCompletion:^(id data) {
            ...
            if (message.integerValue == 1) {
                NSError *error = nil;
                CoachDetail *coacDetail = [MTLJSONAdapter modelOfClass:CoachDetail.class fromJSONDictionary:dataParam[@"data"] error:&error];
                DYNSLog(@"error = %@",error);
                weakSelf.detailModel = coacDetail;
                DYNSLog(@"data = %@",weakSelf.detailModel);
                
                [weakSelf.tableView reloadData];
                [self.tableHeadImageView sd_setImageWithURL:[NSURL
                    URLWithString:weakSelf.detailModel.headportrait.originalpic]
...
- (CGFloat)tableView:(UITableView *)tableView estimatedHeightForHeaderInSection:
    (NSInteger)section{
    if (section == 1) {
        return 32;
    }
